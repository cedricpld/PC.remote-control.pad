...
// Helper function to get a connected Yeelight instance
function getConnectedYeelight(ip: string): Promise<any> {
    return new Promise((resolve, reject) => {
        const light = new Yeelight();
        const timeout = setTimeout(() => {
            light.disconnect();
            reject(new Error('Yeelight connection timed out.'));
        }, 5000);
