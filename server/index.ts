...
app.post("/api/restart-server", (req, res) => {
  ...
  async function getXiaomiData(selector: string) {
    try {
      ...
      console.log(`Valeur brute extraite pour ${selector}: "${rawValue}"`);
      // Remplace la virgule par un point, puis ne garde que les chiffres et le point.
      ...
      console.error(`Erreur détaillée lors de la récupération des données de Xiaomi pour le sélecteur ${selector}:`, error);
