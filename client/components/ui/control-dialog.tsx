      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4 max-h-[60vh] overflow-y-auto pr-4">
          ...
          {formData.actionType === 'yeelight' && <>
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="yeelightIp" className="text-right">IP Ampoule</Label><Input id="yeelightIp" value={formData.yeelightConfig?.ip || ""} onChange={e => setFormData({ ...formData, yeelightConfig: { ...formData.yeelightConfig!, ip: e.target.value }})} className="col-span-3" /></div>
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="yeelightAction" className="text-right">Action</Label><Select value={formData.yeelightConfig?.action} onValueChange={(value: 'toggle'|'on'|'off') => setFormData({ ...formData, yeelightConfig: { ...formData.yeelightConfig!, action: value }})}><SelectTrigger className="col-span-3"><SelectValue placeholder="Choisir une action" /></SelectTrigger><SelectContent><SelectItem value="toggle">Toggle</SelectItem><SelectItem value="on">On</SelectItem><SelectItem value="off">Off</SelectItem></SelectContent></Select></div>
          </>}

          {formData.actionType === 'slider' && <>
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="sliderApi" className="text-right">Endpoint API</Label><Input id="sliderApi" value={formData.sliderConfig?.apiEndpoint || ""} onChange={e => setFormData({ ...formData, sliderConfig: { ...formData.sliderConfig!, apiEndpoint: e.target.value }})} className="col-span-3" /></div>
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="sliderMin" className="text-right">Min</Label><Input id="sliderMin" type="number" value={formData.sliderConfig?.min || 0} onChange={e => setFormData({ ...formData, sliderConfig: { ...formData.sliderConfig!, min: parseInt(e.target.value) || 0 }})} className="col-span-3" /></div>
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="sliderMax" className="text-right">Max</Label><Input id="sliderMax" type="number" value={formData.sliderConfig?.max || 100} onChange={e => setFormData({ ...formData, sliderConfig: { ...formData.sliderConfig!, max: parseInt(e.target.value) || 100 }})} className="col-span-3" /></div>
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="sliderUnit" className="text-right">Unité</Label><Input id="sliderUnit" value={formData.sliderConfig?.unit || ""} onChange={e => setFormData({ ...formData, sliderConfig: { ...formData.sliderConfig!, unit: e.target.value }})} className="col-span-3" /></div>
          </>}

          {formData.actionType === 'statusDisplay' && <>
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="statusApi" className="text-right">Endpoint API</Label><Input id="statusApi" value={formData.statusDisplayConfig?.apiEndpoint || ""} onChange={e => setFormData({ ...formData, statusDisplayConfig: { ...formData.statusDisplayConfig!, apiEndpoint: e.target.value }})} className="col-span-3" /></div>
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="statusUnit" className="text-right">Unité</Label><Input id="statusUnit" value={formData.statusDisplayConfig?.labelUnit || ""} onChange={e => setFormData({ ...formData, statusDisplayConfig: { ...formData.statusDisplayConfig!, labelUnit: e.target.value }})} className="col-span-3" /></div>
            <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="statusInterval" className="text-right">Intervalle (ms)</Label><Input id="statusInterval" type="number" value={formData.statusDisplayConfig?.updateIntervalMs || 2000} onChange={e => setFormData({ ...formData, statusDisplayConfig: { ...formData.statusDisplayConfig!, updateIntervalMs: parseInt(e.target.value) || 2000 }})} className="col-span-3" /></div>
          </>}
